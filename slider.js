...
function sliderFactory(d) {
  ...
  function drawSlider(p) {
    ...
    function init() {
      ...
      handle.append("circle").attr("r", 12);
      if (label) {
        addLabel(handle);
      }
      ...
      function dragSlider(svg) {
        ...
        if (orient == "vertical") {
          ...
          overlay.attr("x2", sliderScale(rounded));
        }
      }
    }
    ///End of init()
    function reposition() {
      ...
      if (orient == "vertical") {
        let track = svg.selectAll(".track").attr("y2", sliderScale.range()[1]);
        let ovelay = svg
      ...
      if (scale) {
        let axisType = getAxis(orient);
        let axis = svg.selectAll(".sliderAxis").call(
          d3[axisType](sliderScale)
            .tickSize(8)
            .tickValues(calculateTicks(range, ticks))
        );
